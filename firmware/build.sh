...
flags=(-mv7M3 --code_state=16 --float_support=vfplib -me --include_path="${script_dir}" --include_path="${sdk}/source/ti/posix/ccs" --include_path="${tool_root}/include" --cmd_file="${tmp_compiler_opt}" --define=DeviceFamily_CC13X0 --define=FIRMWARE_VERSION_CODE=${package_version})
"${tool}" "${flags[@]}" -c "${script_dir}/CC1310_LAUNCHXL.c" "${script_dir}/CC1310_LAUNCHXL_fxns.c" "${script_dir}/RFQueue.c" "${script_dir}/cli_core.c" "${script_dir}/cli_radio.c" "${script_dir}/cli_bootloader.c" "${script_dir}/cli_task_cc1310.c" "${script_dir}/main_tirtos.c" "${script_dir}/firmware.c" "${script_dir}/firmware_mode.c" "${script_dir}/firmware_startup.c" "${script_dir}/firmware_tx.c" "${script_dir}/rf_packet_queue.c" "${script_dir}/i2c_slave.c" "${script_dir}/boot_api.c" "${script_dir}/boot_nonrom_startup.c" "${script_dir}/smartrf_settings/smartrf_settings.c"
"${tool}" "${flags[@]}" -c "${config_source}" --obj_directory="${build_dir}" --output_file=boot_release_cfg.obj
"${tool}" -mv7M3 --code_state=16 --float_support=vfplib -me -z --rom_model -m firmware.map -i"${sdk}/source" -i"${sdk}/kernel/tirtos/packages" -i"${tool_root}/lib" -o firmware.out CC1310_LAUNCHXL.obj CC1310_LAUNCHXL_fxns.obj RFQueue.obj cli_core.obj cli_radio.obj cli_bootloader.obj cli_task_cc1310.obj main_tirtos.obj firmware.obj firmware_mode.obj firmware_startup.obj firmware_tx.obj rf_packet_queue.obj i2c_slave.obj boot_api.obj boot_nonrom_startup.obj smartrf_settings.obj "${script_dir}/boot_app.cmd" -lti/display/lib/display.aem3 -lti/grlib/lib/ccs/m3/grlib.a -lthird_party/spiffs/lib/ccs/m3/spiffs_cc26xx.a -lti/drivers/rf/lib/rf_multiMode_cc13x0.aem3 -lti/drivers/lib/drivers_cc13x0.aem3 -lti/dpl/lib/dpl_cc13x0.aem3 -l"${tmp_cmd}" -lti/devices/cc13x0/driverlib/bin/ccs/driverlib.lib -llibc.a
